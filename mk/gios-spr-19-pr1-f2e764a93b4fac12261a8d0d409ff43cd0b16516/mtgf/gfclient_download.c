...
/* COMMAND LINE OPTIONS ============================================= */
  ...
  /* Making the requests, and put the requests into the queue*/
  for(i = 0; i < nrequests * nthreads; i++){
    ...
    /* request struct, including server, port number and path*/
    steque_node* node;
  ...
  /* wait all the worker threads to complete*/
  for(t=0; t<nthreads; t++){
...
void *threadFunc(void *pArg){
  ...
  while(total_requests>0){
    ...
    /* proceed the request, send request to server, and receive file from server*/
    if(req_obj!=NULL){
